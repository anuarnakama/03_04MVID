...
void render(const Geometry& floor, const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light,
    ...
    //Representacion grafica de las luces puntuales
    ...
    //Representacion grafica de las luces tipo spot
    ...
    for (uint32_t i = 0; i < nPointsLights; i++) {
        ...
        s_phong.set((lightName + "ambient").c_str(), 0.1f, 0.0f, 0.0f);
    ...
    for (uint32_t i = 0; i < nSpotsLights; i++) {
        ...
        s_phong.set((lightName + "cutOff").c_str(), cutOff);
        s_phong.set((lightName + "outerCutOff").c_str(), outerCutOff);
        s_phong.set((lightName + "ambient").c_str(), 0.1f, 0.1f, 0.1f);
        s_phong.set((lightName + "diffuse").c_str(), 0.5f, 0.5f, 0.5f);
        s_phong.set((lightName + "specular").c_str(), 0.6f, 0.6f, 0.6f);
        s_phong.set((lightName + "constant").c_str(), 1.0f);
        s_phong.set((lightName + "linear").c_str(), 0.2f);
