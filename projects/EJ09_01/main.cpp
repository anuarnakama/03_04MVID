...
void render(const Geometry& floor, const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light,
    const Texture& t_albedo, const Texture& t_specular, const Geometry& teapot) {
    ...
    float radio = sqrt(lightPos.x * lightPos.x + lightPos.z * lightPos.z);

    lightPos.x = radio * cos(glfwGetTime() * glm::radians(30.0f));
    lightPos.z = radio * sin(glfwGetTime() * glm::radians(30.0f));
...

    {
        ...
        model = glm::rotate(model, -glm::pi<float>() * 0.5f, glm::vec3(1, 0, 0));
...
int main(int, char* []) {
    ...
    while (window->alive()) {
        ...
        render(quad, cube, sphere, s_phong, s_light, t_albedo, t_specular, teapot);
        window->frame();
    }
