...
glm::vec3 spotLightDir = glm::normalize(glm::vec3(-2.0f, -1.0f, -2.0f));
...
void render(const Geometry& quad, const Geometry& cube, const Geometry& sphere,
    ...
    const glm::mat4 lightProjection = glm::perspective(glm::radians(80.0f), k_shadow_width * 1.0f / k_shadow_height, k_shadow_near, k_shadow_far);
    const glm::mat4 lightView = glm::lookAt(lightPos, lightPos + spotLightDir, glm::vec3(0.0f, 1.0f, 0.0f));
...
//SECOND PASS
    ...
    float outerCutOff = glm::cos(glm::radians(80.0f));
    s_phong.set("light.position", lightPos);
    s_phong.set("light.direction", spotLightDir);
    s_phong.set("light.cutOff", cutOff);
    s_phong.set("light.outerCutOff", outerCutOff);
    s_phong.set("light.ambient", 0.1f, 0.1f, 0.1f);
    s_phong.set("light.diffuse", 0.5f, 0.5f, 0.5f);
    s_phong.set("light.specular", 0.6f, 0.6f, 0.6f);
